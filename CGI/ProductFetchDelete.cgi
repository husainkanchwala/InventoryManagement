...
my $sth = $dbh->prepare(" SELECT p.sku,c.name,v.name,p.vendorModel,p.description,p.features,p.cost,p.retail,p.image from product p,category c,vendor v where sku=? and p.catID=c.id and p.venID=v.id;");
$sth->execute($sku);
